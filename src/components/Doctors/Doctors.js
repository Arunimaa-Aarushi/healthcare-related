import React from 'react';
import img1 from './doctor-1.jpeg';
import img2 from './doctor-2.jpeg';
import img3 from './doctor-3.jpeg';

const Doctors = () => {
    return (
        <div className="p-5">
            <div className="container">
                <h2 className="text-center mb-4 text-danger">Our Doctors</h2>
                <div className="row row-cols-lg-3 row-cols-md-2 row-cols-sm-1">
                    <div className="col">
                        <div className="card h-100 shadow-lg border-1">
                            <img height="200px" src={img1} className="card-img-top" alt="..." />
                                <div  className="card-body">
                                <h5 className="card-title">Dr. Alen Marken</h5>
                                </div>
                        </div>
                    </div>
                    <div className="col">
                        <div className="card h-100 shadow-lg border-1">
                            <img height="200px" src={img2} className="card-img-top" alt="..." />
                            <div className="card-body">
                                <h5 className="card-title">Dr. Samira Sans</h5>
                            </div>
                        </div>
                    </div>
                    <div className="col">
                        <div className="card h-100 shadow-lg border-1">
                            <img height="200px" src={img3} className="card-img-top" alt="..." />
                            <div className="card-body">
                                <h5 className="card-title">Dr. Jign Postine</h5>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    );
};

export default Doctors;